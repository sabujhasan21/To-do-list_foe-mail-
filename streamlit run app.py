    ...
    for t in tasks:
        ...
        if end_date:
            try:
                ...
                if end_dt <= today and status != "Completed":
                    notify(f"⚠ Pending Task: {t.get('Task')}\nEnd Date: {end_date}", "warning")
            except:
                continue
...
def csv_export_page():
    ...
    filtered = [t for t in tasks if start <= date.fromisoformat(t.get("Start","2025-01-01")) <= end]
    if not filtered:
...
def main():
    ...
    menu_choice = st.sidebar.radio("", ["Add Task","Active Tasks","Completed Tasks","CSV Export"])
